...
const {hEthers} = require("@nomiclabs/hardhat-ethers");
const {ethers} = require("ethers");
const {expect, describe, beforeEach, it} = require("chai");
...
const getBalance = ethers.BrowserProvider.getBalance;
...
describe("Exchange", () => {
    ...
    beforeEach(async () => {
        [owner, user] = await ethers.BrowserProvider.getSigners();

        const Token = await hEthers.getContractFactory("token");
        token = await Token.deploy("Token", "TKN", toWei(1000000));

        const Exchange = await hEthers.getContractFactory("Exchange");
        exchange = await Exchange.deploy(token.address);
    });

    it("is deployed", async () => {
        expect(await exchange.deploy()).to.equal(exchange);
    });
